...
@section('dashboard_content')
    ...
    <div class="container py-4">
        ...
        <div class="page-path mb-3">
            ...
            <span class="text-muted">ویرایش مقاله</span>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('dashboard.manage-articles.update', $article->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="authors" value="{{ old('authors', $article->authors) }}" class="form-control" required>
                        </div>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="type" class="form-select" required>
                                <option value="national" {{ old('type', $article->type) == 'national' ? 'selected' : '' }}>بومی</option>
                                <option value="international" {{ old('type', $article->type) == 'international' ? 'selected' : '' }}>بین‌المللی</option>
                            </select>
                        </div>

                        <div class="col-md-3 mb-3">
                            <label class="form-label">سال انتشار <span class="text-danger">*</span></label>
                            <input type="text" name="publication_year" value="{{ old('publication_year', $article->publication_year) }}" class="form-control" required>
                        </div>

                        <div class="col-md-3 mb-3">
                            <label class="form-label">وضعیت</label>
                            <select name="status" class="form-select">
                                <option value="pending" {{ old('status', $article->status) == 'pending' ? 'selected' : '' }}>در انتظار تایید</option>
                                <option value="approved" {{ old('status', $article->status) == 'approved' ? 'selected' : '' }}>تایید شده</option>
                                <option value="rejected" {{ old('status', $article->status) == 'rejected' ? 'selected' : '' }}>رد شده</option>
                            </select>
                        </div>

                        <div class="col-md-3 mb-3">
                            <label class="form-label">قیمت (تومان)</label>
                            <input type="number" name="price" value="{{ old('price', $article->price) }}" class="form-control" min="10000">
                        ...
                        <div class="col-md-3 mb-3">
                            <div class="form-check mt-4">
                                <input type="checkbox" name="is_free" class="form-check-input" id="is_free" value="1" 
                                    {{ old('is_free', $article->is_free) ? 'checked' : '' }}>
                                <label class="form-check-label" for="is_free">رایگان</label>
                            </div>
                        </div>
                        <br>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">فایل اصلی مقاله</label>
                            @if($article->file)
                                <div class="mb-1">
                                    <a href="{{ asset('storage/' . $article->file) }}" target="_blank" class="btn btn-sm btn-outline-primary">
                                        مشاهده فایل فعلی
                                    </a>
                                </div>
                            @endif
                            <input type="file" name="file" class="form-control" accept=".pdf,.doc,.docx">
                            <small class="text-muted">فایل جدید را انتخاب کنید تا جایگزین شود</small>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            @if($article->abstract_file)
                                <div class="mb-1">
                                    <a href="{{ asset('storage/' . $article->abstract_file) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                                        مشاهده فایل چکیده فعلی
                                    </a>
                                </div>
                            @endif
