...
@section('dashboard_style')
    @if ( config('app.env') === 'local' )
        <link href="{{ asset('css/dashboard/manage-projects/reports.css')}}" rel="stylesheet">
    @else
        <link href="{{ asset('css/dashboard/manage-projects/reports.css') }}?v={{ filemtime(config('app.server_css_files_path') . '/dashboard/manage-projects/reports.css') }}" rel="stylesheet">
    @endif
@endsection
...
@section('dashboard_content')
    ...
    <div class="container py-4">
        ...
        <div class="px-5 mb-4">
            <h5>متقاضی: {{ $project->applicant->name }}</h5>
            <h5>مسئول اصلی پروژه: {{ $project->primaryCoach->name }}</h5>
            <h5>مسئول دوم پروژه: {{ $project->secondaryCoach->name ?? '-' }}</h5>
        </div>
