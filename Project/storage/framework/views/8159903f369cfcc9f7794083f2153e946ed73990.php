...
<?php $__env->startSection('master_content'); ?>
    ...
    <div class="content-section">
        <div class="container">
            ...
            <div class="filter-section">
                <form method="GET" action="<?php echo e(route('scientific.articles-international')); ?>" class="row g-3" id="filterForm">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2">
                        <select name="type_filter" class="form-select" id="typeFilter">
                            ...
                            <option value="national" <?php echo e(request('type_filter') == 'national' ? 'selected' : ''); ?>>بومی</option>
                            <option value="international" <?php echo e(request('type_filter') == 'international' ? 'selected' : ''); ?>>بین‌المللی</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2 d-flex gap-2">
                        ...
                        <a href="<?php echo e(route('scientific.articles-international')); ?>" class="btn btn-secondary btn-reset">
                            پاک کردن
                        </a>
                    </div>
                </form>
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <span class="articles-count">
                    نمایش <?php echo e($articles->firstItem() ?? 0); ?> تا <?php echo e($articles->lastItem() ?? 0); ?> از <?php echo e($articles->total()); ?> مقاله
                </span>
            </div>
            ...
            <?php if($articles->isEmpty()): ?>
                <div class="empty-state text-center py-5">
                    ...
                    <a href="<?php echo e(route('scientific.articles-international')); ?>" class="btn btn-outline-primary mt-3">
                        نمایش همه مقالات
                    </a>
                </div>
            <?php else: ?>
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="article-card">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <span class="badge-year"><?php echo e($article->publication_year ?? '-'); ?></span>
                                    <span class="badge-type <?php echo e($article->type === 'national' ? 'badge-national' : 'badge-international'); ?>">
                                        <?php echo e($article->type === 'national' ? 'بومی' : 'بین‌المللی'); ?>

                                    </span>
                                </div>
                                
                                <div class="mb-2">
                                    <?php if($article->is_free): ?>
                                        <span class="badge-price badge-free">رایگان</span>
                                    <?php else: ?>
                                        <span class="badge-price badge-paid"><?php echo e(number_format($article->price)); ?> تومان</span>
                                    <?php endif; ?>
                                </div>

                                <h5 class="mt-1"><?php echo e(Str::limit($article->title, 60)); ?></h5>
                                <p class="authors">نویسندگان: <?php echo e(Str::limit($article->authors ?? '-', 60)); ?></p>

                                <?php if($article->description): ?>
                                    <p class="article-description"><?php echo e(Str::limit($article->description, 100)); ?></p>
                                <?php endif; ?>
                                ...
                                <?php if($article->keywords): ?>
                                    <div class="article-keywords">
                                        <?php $__currentLoopData = explode(',', $article->keywords); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $keyword): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <span class="keyword"><?php echo e(trim($keyword)); ?></span>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </div>
                                <?php endif; ?>

                                <div class="card-footer-actions">
                                    <a href="<?php echo e(route('scientific.article.show', $article->id)); ?>" class="btn btn-outline-primary btn-sm">
                                        مشاهده مقاله
                                    </a>
                                    
                                    <?php if(auth()->guard()->check()): ?>
                                        <?php if(!$article->is_free): ?>
                                            <?php if($article->isPurchasedByUser(auth()->id())): ?>
                                                <span class="badge bg-info text-white p-2">خریداری شده</span>
                                            <?php elseif($inCart): ?>
                                                <form method="POST" action="<?php echo e(route('shop.remove-from-cart-article', $article->id)); ?>" class="d-inline remove-form">
                                                    <?php echo csrf_field(); ?>
                                            ...
                                            <?php else: ?>
                                                <form method="POST" action="<?php echo e(route('shop.add-to-cart', $article->id)); ?>" class="d-inline add-form">
                                                    <?php echo csrf_field(); ?>
                                            ...
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <a href="<?php echo e(route('shop.download-purchased', $article->id)); ?>" class="btn btn-success btn-sm">
                                                دانلود
                                            </a>
                                        <?php endif; ?>
                                    <?php else: ?>
    ...
    <div class="modal-cart-overlay" id="cartModal">
        <div class="modal-cart-box">
            ...
            <div class="btn-group">
                <a href="<?php echo e(route('shop.cart')); ?>" class="btn btn-success">رفتن به سبد خرید</a>
    ...
    <?php if(auth()->guard()->check()): ?>
        <?php
            $cartCount = \App\Models\Order::where('user_id', auth()->id())
                ...
                ->first()?->items_count ?? 0;
        ?>
        <a href="<?php echo e(route('shop.cart')); ?>" class="floating-cart" id="floatingCart">
            🛒 سبد خرید
            <?php if($cartCount > 0): ?>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\wamp64\www\2\ASA\version_3\Project\resources\views/scientific/articles-international.blade.php ENDPATH**/ ?>